...
class Service
{
    ...
    public static function local($file, $extend = [])
    {
        ...
        $uploadFile = $file->rule('uniqid')->validate(['size' => 102400000, 'ext' => 'zip,zhapp'])->move($addonsTempDir);
        if (!$uploadFile) {
            // 上传失败获取错误信息
...
EOD;
    ...
    public static function enable($name, $force = false)
    {
        ...
        //备份冲突文件
        if (config('site.backup_global_files')) {
        ...
        if (config('site.addon_pure_mode')) {
            // 删除插件目录已复制到全局的文件
    ...
    public static function disable($name, $force = false)
    {
        ...
        if (config('site.addon_pure_mode') || !$list) {
    ...
    {
        return is_dir($dir) or (self::mkdir(dirname($dir)) and mkdir($dir, 0755));
    }
}
